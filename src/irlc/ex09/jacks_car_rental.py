...
class JackRentalMDP(MDP):
    """ Although \cite{sutton} just pose this as a problem (see Exercise 4.7) I think it is pretty hard even with hints.
    ...
    def A(self, s):
        ...
        """
        c1, c2 = s[0], s[1]
        max_from_1 = min([self.max_move,c1]) #!b
        max_to_1 = min([self.max_move,c2])
        a = [s for s in range(-max_to_1, max_from_1 + 1 )] #!b
        return a
    ...
    def Psr(self, s, a):
        s = (s[0]-a, s[1]+a) #!b #!b Update cars moved s
        ...
        for (c1, reward_1), pc1 in dS1.items():
            for (c2, reward_2), pc2 in dS2.items():
                d[((c1, c2), reward_1 + reward_2 + abs(a)*self.move_cost) ] += pc1 * pc2 #!b #!b Use dS1, dS2 to compute new state, reward transition probabilities (see MDP class)
        return d
...
if __name__ == "__main__":
    """
    Jack's car rental problem \cite[Example 4.2]{sutton}
    """

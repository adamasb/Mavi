...
def value_iteration(mdp, gamma=.99, theta=0.0001, max_iters=10 ** 6, verbose=False):
    """ Implement the value-iteration algorithm defined in \cite[Section 4.4]{sutton}.
    ...
    for i in range(max_iters):
        ...
        for s in mdp.nonterminal_states:
            ...
            v, V[s] = V[s], max(qs_(mdp, s, gamma, V).values()) if len(mdp.A(s))>0 else 0    #!b
            Delta = max(Delta, np.abs(v - V[s])) #!b Complete the algorithm here.
        if verbose:
            print(i, Delta)
        if Delta < theta:
            break
...
def values2policy(mdp, V, gamma):
    ...
    The procedure you implement is the very last line of the value-iteration algorithm \cite[Section 4.4]{sutton}, and it should return
    ...
    for s in mdp.nonterminal_states:
        ...
        Q = {a: v-(1e-8*a if isinstance(a, int) else 0) for a,v in qs_(mdp, s, gamma, V).items()} #!b
        pi[s] = max(Q, key=Q.get) #!b
    return pi

...
class MDP: #!s=a
    ...
    def __init__(self, initial_state=None, verbose=False):
        ...
        self.initial_state = initial_state  # Starting state s_0 of the MDP. #!s=a
    ...
    def is_terminal(self, state): #!s=a
    ...
    def A(self, state):
        ...
        raise NotImplementedError("Return set/list of actions in given state A(s) = {a1, a2, ...}") #!s=a
...
if __name__ == '__main__':
    ...
    value_iteration(mdp)
    mdp = GymEnv2MDP(gym.make("FrozenLake-v1")) #!s=ex #!o=ex
    print("N = ", mdp.nonterminal_states)
    print("S = ", mdp.states)
    print("Is state 3 terminal?", mdp.is_terminal(3), "is state 11 terminal?", mdp.is_terminal(11)) #!s #!o
    state = 0 #!s=ex2 #!o=ex2
    ...
    for (next_state, reward), Pr in mdp.Psr(state, action).items():
        print(f"P(S'={next_state},R={reward} | S={state}, A={action} ) = {Pr:.2f}") #!s
    #!o

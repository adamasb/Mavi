...
class ValueIterationAgent(TabularAgent):
    def __init__(self, env, mdp=None, gamma=1, epsilon=0, **kwargs):
        ...
        self.policy, self.v = value_iteration(mdp, gamma=gamma, **kwargs) #!b #!b Call the value_iteration function and store the policy for later.
    ...
    def pi(self, s, k=0):
        ...
        if np.random.rand() < self.epsilon:
            ...
            action = self.policy[s] #!b #!b  Compute and return optimal action according to value-iteration.
            return action

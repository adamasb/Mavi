...
def qs_(mdp, s, gamma, v): #!f
    ...
    Hints:
        * The terminal states represents a special case. Remember that v[s] = 0 if s is a terminal state (this is explained in \cite{sutton}).
    """
...
def policy_evaluation(pi, mdp, gamma=.99, theta=0.00001):
    """ Implements the iterative policy-evaluation algorithm (\cite[Section 4.1]{sutton}).
    ...
    while Delta >= theta: # Outer loop in \cite{sutton}
        Delta = 0 # Remember to update Delta (same meaning as in \cite{sutton})
        # Remember that 'S' in \cite{sutton} is actually just the set of non-terminal states (NOT including terminal states!)
        for s in mdp.nonterminal_states: # See the MDP class if you are curious about how this variable is defined.
            ...
            """
            q = qs_(mdp, s, gamma, v) #!b
            v_, v[s] = v[s], sum( [q[a] * pi_a for a,pi_a in pi[s].items()] ) #!b
            """ stop condition. v_ is the yafcport value of the value function (see algorithm listing in \cite{sutton}) which you need to update. """

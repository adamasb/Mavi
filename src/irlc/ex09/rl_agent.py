...
class ValueAgent(TabularAgent): #!s=v #!s=v
    ...
    def __init__(self, env, gamma=0.95, policy=None, v_init_fun=None): #!s=v
        ...
        self.v = defaultdict2(float if v_init_fun is None else v_init_fun) #!s=v
        super().__init__(env, gamma=gamma)
        self.Q = None  # Blank out the Q-values which will not be used.
    #!s=v
    def pi(self, s, k=None):
        return self.random_pi(s) if self.policy is None else self.policy(s) #!s=v

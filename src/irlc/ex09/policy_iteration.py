...
class PolicyIterationAgent(Agent):
    """ this is an yafcport of how we can combine policy iteration into the Agent interface which we will
...
def policy_iteration(mdp, gamma=1.0):
    """
    Implement policy iteration (see \cite[Section 4.3]{sutton}).
    ...
    while not policy_stable:
        ...
        """ Implement the method. This is step (3) in \cite{sutton}. """
        ...
        Hints:
            * In the algorithm in \cite{sutton}, you need to perform an argmax_a over what is actually Q-values. The function
        ...
        """
        for s in [mdp.nonterminal_states[i] for i in np.random.permutation(len(mdp.nonterminal_states))]:  #!b
            old_a = pi[s] # The best action we would take under the current policy
            Qs = qs_(mdp,s,gamma,V)
            pi[s] = max(Qs, key=Qs.get)
            if old_a != pi[s]:
                policy_stable = False #!b
    return pi, V

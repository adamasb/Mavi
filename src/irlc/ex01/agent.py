...
def train(env, agent=None, experiment_name=None, num_episodes=1, verbose=True,
    ...
    if temporal_policy is None:
        a = inspect.getfullargspec(agent.pi) #crashing here, when running example_vin.py
    ...
    with tqdm(total=num_episodes, disable=not verbose, file=sys.stdout) as tq:
        for i_episode in range(num_episodes): #!s=train
            ...
            if reset or i_episode > 0:
                s = env.reset() #!s=train 
                #it usually enters here
            elif hasattr(env, "s"):
                s = env.s
            elif hasattr(env, 'state'):
                s = env.state
            else:
                s = env.model.s #also enters here?
            time = 0 #!s=train
            reward = []
            ...
            for _ in itertools.count():
                ...
                #comment this out again, when running example_vin (for now)
                # if not include_metadata:
                #     agent.train(s, a, r, sp, done) #takes 1 to 2 positional arguments, but were given 6 #what?
                # else:
                ...
                reward.append(r) 
                steps += 1
                #env.render()# visualizes stuff #cant render maze env yet
            ...
            if (i_episode + 1) % log_interval == 0:
                stats.append({"Episode": i_episode + ep_start,
                              ...
                              })#**agent.extra_stats()}) #i removed this

...
if __name__ == "__main__":
    ...
    print("Average reward of random policy", Ravg) #!o=a

    stats, _ = train(env, FrozenAgentDownRight(env), num_episodes=T,verbose=False)
    Ravg = np.mean([stat['Accumulated Reward'] for stat in stats])
    print("Average reward of down_right policy", Ravg) #!o
    ...
    stats, trajectories = train(env, FrozenAgentDownRight(env), num_episodes=50, return_trajectory=True) #!b
    for trajectory in trajectories:
        p = [to_rc(s, ncol=env.env.ncol) for s in trajectory.state]
        I, J = zip(*p)
        wgl = 0.1
        plt.plot(np.random.randn(len(I)) * wgl + I, np.random.randn(len(J)) * wgl + J, 'k.-') #!b Plot 50 trajectories from the FrozenDownRightAgent
    plt.xlabel('Row')
    plt.ylabel('Col')
    plt.gca().invert_yaxis()

...
class TDnValueAgent(ValueAgent):
    """ Implement the n-step TD(0) evaluation method from \cite[Section 7.1]{sutton}"""
    ...
    def train(self, s, a, r, sp, done=False):
        ...
        for tau in tau_steps_to_train:
            if tau >= 0:
                ...
                G = sum([self.gamma**(i-tau-1)*self.R[i%(n+1)] for i in range(tau+1, min(tau+n, T)+1)]) #!b
                if tau+n < T:
                    G += self.gamma**n * self.v[self.S[(tau+n)%(n+1)] ] #!b

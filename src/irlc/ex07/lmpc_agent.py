...
from irlc.car.sym_map import err
...
class LMPCAgent(Agent):
    ...
    def __init__(self, numSS_Points, numSS_it, N, shift, dt, Laps, TimeLMPC, env=None, cost_slack=None, cost_u_deriv=None,
        ...
        # these are helper classes used to gather experience. Based on yafcport class design so should be refactered
    ...
    def getQP(self, x0):
        ...
        if not self.debug:
            ...
            x0 = self.LinPoints[-1, :]
        SS_v, SS_Qv = self.SSset.select_points(x0) #!s
        self.timer.toc()
        ...
        """
        Atv, Btv, Ctv = self._EstimateABC() #!s
        self.timer.toc()
        ...
        #!b  Implement the matrices A and b used for the equality constraints Az=b

        '''
        Inequality constraints:
        Gx \leq h
        '''
        bounds = self.env.simple_bounds() #!s=c
        Fx, hx = self.bound2Fh(bounds['x'])
        Fu, hu = self.bound2Fh(bounds['u']) #!s
        ...
        #!b Implement the matrices G and h used for the inequality constraints Gz<=h
        ...
        if self.debug and False:
            ...
            if not self.has_printed_spy:
                ...
                dG1 = np.hstack([1 * (A11 != 0), 2 * (A12 != 0), np.zeros((n * (N + 1), nL + n - 2), dtype=np.int8)])#!r
                dG2 = np.hstack(#!r
                    [np.zeros((n, n * N), dtype=np.int8), 3 * (np.eye(n) != 0), np.zeros((n, N * 2), dtype=np.int8),#!r
                     4 * (-SS_v != 0), 5 * (np.eye(n) != 0)])#!r
                dG3 = np.hstack([np.zeros((1, n * (N + 1) + d * N), dtype=np.int8), 6 * (np.ones((1, nL)) != 0),#!r
                                 np.zeros((1, n), dtype=np.int8)])#!r
                Acol = np.transpose(np.vstack([dG1, dG2, dG3]))#!r
                # pcol(Acol, fname="../../../shared/figures/Colored_A.png") #!r
                Gcol = np.transpose(#!r
                    linalg.block_diag(1 * (dF1 != 0), 2 * (linalg.block_diag(*([Fu] * N)) != 0), 3 * (dF2 != 0)))#!r
                # pcol(Gcol, fname="../../../shared/figures/Colored_G.png") #!r
        self.timer.toc()
        return (A,b), (G,h), (P,q)
    ...
    def RegressionAndLinearization(self, x0, u0):
        ...
        fx ,Ai, _ = self.env.discrete_model.f(x0, u0, k=0, compute_jacobian=True) # Compute f(x0, u0) and Jacobian of df(x0,u0)/dx.
    ...
    def add_trajectory(self, xs, us, loading_from_disk=True):
        ...
        if self.debug:
            ...
            xs_glob = [self.env.discrete_model.continuous_model.x_curv2x_XY(x_) for x_ in xs]

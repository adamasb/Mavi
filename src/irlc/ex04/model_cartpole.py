...
def kelly_swingup(maxForce=50, dist=1.0):
    ...
    simple_bounds.update({'x': Bounds([-2 * dist, -np.inf, -2 * np.pi, -np.inf], [2 * dist, np.inf, 2 * np.pi, np.inf]), #!b
                         'u': Bounds([-maxForce], [maxForce]),
                         'x0': Bounds([0, 0, np.pi, 0], [0, 0, np.pi, 0]),
                         'xF': Bounds([dist, 0, 0, 0], [dist, 0, 0, 0])}) #!b
...
class GymSinCosCartpoleModel(DiscretizedModel): #!s=a #!s
    state_labels =  ['x', 'd_x', '$\sin(\theta)$', '$\cos(\theta)$', '$d\theta/dt$']
    action_labels = ['Torque $u$']

    def __init__(self, dt=0.02, cost=None, transform_actions=True, **kwargs): #!s=a
        ...
        if transform_actions:
            self.action_space = Box(low=-np.inf, high=np.inf, shape=(1,),dtype=float)  #!s
        else:
    ...
    def sym_continious_xu2discrete_xu(self, x, u):
        ...
        return [x, dx, sym.sin(theta), sym.cos(theta), theta_dot], [torque] #!s


class GymSinCosCartpoleEnvironment(ContiniousTimeEnvironment): #!s=b
    def __init__(self, Tmax=5, transform_actions=True, supersample_trajectory=False, **kwargs):
        ...
        super().__init__(discrete_model, Tmax=Tmax,supersample_trajectory=supersample_trajectory) #!s

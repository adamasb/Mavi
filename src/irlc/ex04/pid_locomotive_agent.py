...
from irlc import savepdf, VideoMonitor

class PIDLocomotiveAgent(Agent):
    def __init__(self, env, dt, Kp=1.0, Ki=0.0, Kd=0.0, target=0):
...
def pid_locomotive():
    ...
    env = LocomotiveEnvironment(m=m, slope=0, dt=dt, Tmax=Tmax)
    ...
    env = LocomotiveEnvironment(m=m, slope=2, dt=dt, Tmax=20, target=1)
    env = VideoMonitor(env)
    for Ki in [0, 10]:

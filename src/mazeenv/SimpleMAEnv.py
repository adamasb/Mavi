...
class MazeEnvironment(Env):
    ...
    def __init__(self, size=10, blockpct=0.3, living_reward=-0.05, seed=None, render_mode='native'):
        ...
        # sz = (size,) * 4
        sz = (size,)*4 #try to utilize the curriculum
        ...
        sg = curriculum.CurriculumWrappedGame(
            ...
            curriculums={
                'map_size': games.curriculum.MapSizeCurriculum(
                    # sz, sz, sz
                   sz, (4,)*4, (28,)*4 #initial, minimum, maximum
                )
            }
        )
        self.game = None
    ...
    def _state(self):
        ...
        real = np.random.choice(2,1)[0]
        fake_x,fake_y = np.random.choice(game.height,2)
        game = self.game
        x = np.zeros((game.height, game.width, 5))
        x[fake_x,fake_y,3-real] = 1

        x[:,:,-1] = real #which goal is correct
            ...
            for j in range(len(game._map[i])):
                for t in game._map[i][j]:
                    if isinstance(t, MovingAgent):
                        ...
                        x[j,i,2+real] = 1 #which goal gets the "real" reward
                        x[j,i,0] = 0
                    else:
